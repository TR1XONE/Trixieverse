...
export function getItemImageUrl(itemName: string): string | null {
    // Generate slug for wildriftfire CDN
    let slug = itemName.toLowerCase()
        .replace(/['\.]+/g, '')
        .replace(/\s+/g, '-');

    // Handle enchant edge cases if necessary (usually they just map directly like 'stasis')
    return `https://www.wildriftfire.com/images/items/${slug}.png`;
}
